...
class Task(
    ...
    private val log = LogProvider.instance.getLogger("Task")
    ...
    fun setState(newState: State) {
        log.i("$this: transition: ${state.name} -> ${newState.name}")
        ...
        when (newState) {
            ...
            State.Finishing -> log.i("$this: finishing at ${scheduler.getTicks()} (run for ${scheduler.getTicks() - (startedAt ?: 0)} ticks)")
            else -> {}
        }
        state = newState
    }
